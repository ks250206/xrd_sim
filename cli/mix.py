...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="CIF から分率を指定して合成/個別の XRD プロファイルを生成します。"
    )
    ...
    parser.add_argument(
        ...
        default="standard",
        help="standard=各相+合成, mix=合成のみ (ミックス専用)",
    )
    parser.add_argument(
        "-o",
        "--output",
        default="xrd_profiles.png",
        help="出力ファイルパス (複数分率の場合は自動でタグ付け)",
    )
    parser.add_argument(
        "--output-format",
    ...
    parser.add_argument(
        "--fractions-auto",
        action="store_true",
        help="分率を自動全探索 (合計 1.0、指定ステップ刻み) で生成する",
    )
    parser.add_argument(
        "--fractions-step",
        type=float,
        default=0.1,
        help="--fractions-auto 時の分率刻み幅 (例: 0.1 で 0,0.1,...,1.0)",
    )
    parser.add_argument(
        "--offset",
        type=float,
